...
-(void) enableDisablePayNowButton{
    if(_isCardNumberValid && _isNameOnCardValid && _isExpiryDateValid && _isCvvNumberValid){
        ...
        [_payNowBtn setBackgroundColor:kLightGrayBgColor];
    }
}
...
-(void) hideAndShowClickButton :(BOOL) aFlag{
    ...
    if(nil == _msgLbl){
        ...
        _msgLbl.allowsDefaultTighteningForTruncation = YES;
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    //NSLog(@"Str(connectionSpecificDataObject) =%@",[[NSString alloc] initWithData:_connectionSpecificDataObject encoding:NSUTF8StringEncoding]);
